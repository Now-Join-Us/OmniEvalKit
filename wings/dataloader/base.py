...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i: int) -> Dict[str, torch.Tensor]:
        ...
        results = dict(input_ids=input_ids, labels=labels)
        if image is not None:
            results.update(image=image)

        if self.data_args.unchange_passing_key is not None:
            results[self.data_args.unchange_passing_key] = sample[self.data_args.unchange_passing_key]
