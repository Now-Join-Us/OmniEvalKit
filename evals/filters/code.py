...
class CodeFilter(object):
    ...
    def apply(self, response, data, question_type=None, **kwargs):
        ...
        entrypoint = data['gold']['entry_point']
        filtered_responses = []
        for r in response:
            sanitized_code = extract_target_code_or_empty(r, entrypoint).strip()
            if not sanitized_code:
                sanitized_code = code_extract(r)
            filtered_responses.append(r)
        
        return {'filtered_response': filtered_responses, 'is_filtered': True}

filter_core = CodeFilter
