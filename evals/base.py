...
class EvalTool(object):
    ...
    def __init__(self,
        ...
        calculate_type='each_then_overall',
        question_type=None, request_type=None,
        **inner_kwargs # 即eval_args
        ):
        ## 构建 filter 模块 ##
        self.filters = []
    ...
    def calculate_score(self, filtered_responses):
        ...
        """
        if self.calculate_type == 'overall':
            ## TODO: 对一整个数据集直接算指标的 ##
    ...
    def estimate_statistic(self, scores):
        ...
        """
        if self.calculate_type == 'each_then_overall':
            ...
            return self.dataset.estimate(scores, categories, sub_categories, **self.inner_kwargs)
        else:
            raise NotImplementedError
    ...
    def filter_answer(self, responses):
        ...
        for f in self.filters:
            for r, entry in zip(responses, self.dataset):
                filtered_responses.append(f.apply(r, entry, self.question_type, **self.inner_kwargs))
